#!/usr/bin/env node

// AppMate CLI v0.2.0
// Instant dev app bootstrapper (React + Vite + Firebase-ready)
// Powered by Nova — now with extra sauce

const fs = require('fs');
const { execSync } = require('child_process');
const path = require('path');
const inquirer = require('inquirer');
const chalk = require('chalk');

(async () => {
  console.log(chalk.cyanBright(`\n🧠  Welcome to AppMate — the Nova-powered dev launcher`));

  const { appName, preset } = await inquirer.prompt([
    {
      type: 'input',
      name: 'appName',
      message: 'What do you want to name your app?',
      default: 'nova-app'
    },
    {
      type: 'list',
      name: 'preset',
      message: 'Choose your stack preset:',
      choices: ['React + Vite', 'Next.js', 'Vanilla + Vite'],
      default: 'React + Vite'
    }
  ]);

  const appDir = path.resolve(process.cwd(), appName);

  if (fs.existsSync(appDir)) {
    console.log(chalk.red(`\n❌ Folder ${appName} already exists. Please choose a different name.`));
    process.exit(1);
  }

  console.log(chalk.green(`\n🚀 Creating ${preset} project in ${appDir}`));
  fs.mkdirSync(appDir);
  process.chdir(appDir);

  if (preset === 'React + Vite') {
    execSync(`npm create vite@latest . -- --template react`, { stdio: 'inherit' });
    execSync(`npm install`, { stdio: 'inherit' });
    execSync(`npm install firebase`, { stdio: 'inherit' });

    const viteConfig = `import { defineConfig } from 'vite';
import react from '@vitejs/plugin-react';

export default defineConfig({
  plugins: [react()],
  server: {
    port: 5173,
    open: true,
    strictPort: true
  },
  resolve: {
    alias: {
      '@': '/src'
    }
  }
});
`;
    fs.writeFileSync('vite.config.js', viteConfig);

    console.log(chalk.blue(`\n🧩 Vite config ready.`));
    console.log(chalk.yellow(`⚙️ Installing Nova assistant panel soon...`));
  }

  // Add Firebase placeholder config
  const firebaseConfig = `// Replace with your Firebase config
export const firebaseConfig = {
  apiKey: '',
  authDomain: '',
  projectId: '',
  storageBucket: '',
  messagingSenderId: '',
  appId: ''
};
`;
  fs.mkdirSync('src/firebase', { recursive: true });
  fs.writeFileSync('src/firebase/config.js', firebaseConfig);

  // Add starter index.html if missing
  const indexHtml = `<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
    <link rel="icon" type="image/svg+xml" href="/vite.svg" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>${appName}</title>
  </head>
  <body>
    <div id="root"></div>
    <script type="module" src="/src/main.jsx"></script>
  </body>
</html>
`;
  if (!fs.existsSync('index.html')) {
    fs.writeFileSync('index.html', indexHtml);
  }

  // Create sample src/main.jsx file
  const mainJsx = `import React from 'react';
import ReactDOM from 'react-dom/client';

const App = () => <h1>🚀 Welcome to ${appName}</h1>;

ReactDOM.createRoot(document.getElementById('root')).render(
  <React.StrictMode>
    <App />
  </React.StrictMode>
);
`;
  fs.mkdirSync('src', { recursive: true });
  fs.writeFileSync('src/main.jsx', mainJsx);

  console.log(chalk.green(`\n✅ Project created successfully!`));
  console.log(chalk.magentaBright(`\n👉 Run these next:`));
  console.log(chalk.white(`   cd ${appName}`));
  console.log(chalk.white(`   npm run dev`));
  console.log(chalk.gray(`\nHappy hacking. Nova is watching. 🦉`));
})();
